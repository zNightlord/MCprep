...
def get_enabled_modules_names() -> List[str]:
    ...
    mcprep_feature_sets = get_prefs_feature_sets()

    enabled_module_names = {fs.module_name for fs in mcprep_feature_sets if fs.enabled}
...
def get_info_dict(feature_set: str):
    ...
    if module and hasattr(module, 'mcprep_info'):
        data = module.mcprep_info
    ...
    if module:
        ...
        if callable(func):
            ...
            try:
                ...
                print(f"MCprep Error: Could not call function '{func_name}' of feature set "
...
def feature_set_items(_scene, _context):
    ...
    items = [
        ('all', 'All', 'All installed feature sets and rigs bundled with MCprep'),
        ('mcprep', 'MCprep default', 'Features bundled with MCprep'),
    ]
...
class MCPREP_OT_AddFeatureSet(bpy.types.Operator):
    bl_idname = "mcprep.add_feature_set"
    bl_label = "Add External Feature Set"
    bl_description = "Add external feature set"
    ...
    def execute(self, context):
        ...
        with ZipFile(bpy.path.abspath(self.filepath), 'r') as zip_archive:
            ...
            fixed_dir = os.path.join(mcprep_config_path, fixed_dirname)
            ...
            zip_archive.extractall(mcprep_config_path)
            ...
            addon_prefs.active_feature_set_index = len(addon_prefs.mcprep_feature_sets)-1

        return {'FINISHED'}


# noinspection PyPep8Naming
class MCPREP_OT_RemoveFeatureSet(bpy.types.Operator):
    bl_idname = "mcprep.remove_feature_set"
    bl_label = "Remove External Feature Set"
    bl_description = "Remove external feature set"
    bl_options = {"REGISTER", "UNDO", "INTERNAL"}
    ...
    def execute(self, context):
        ...
        feature_set_list = addon_prefs.mcprep_feature_sets
        active_idx = addon_prefs.active_feature_set_index
        active_fs: 'MCprepFeatureSets' = feature_set_list[active_idx]
        ...
        mcprep_config_path = get_install_path()
        if mcprep_config_path:
            set_path = os.path.join(mcprep_config_path, active_fs.module_name)
